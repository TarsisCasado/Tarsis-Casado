...
function ensureHeaders(sheetName) {
  ...
  // Se a aba USUARIOS já existia sem lojas/abas, apenas ADICIONA as colunas novas
  ...
  if (key === 'USUARIOS') {
    ...
    return changed ? getHeaders('USUARIOS') : headers;
  }

  const matches = headers.every((h, i) => currentHeaders[i] === h);
  if (!matches) {
    sh.clear();
    const range = sh.getRange(1, 1, 1, headers.length);
    range.setValues([headers]);
